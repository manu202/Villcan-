...
**What this does and does NOT mean:** `main` is now the single codebase — every Current status line above marked "in `main` since `016b773`" is accurate as of this commit, no exceptions. It does **not** mean the M-4/monitoring migrations reached production (still git-only, per the entry above — a separate, not-yet-requested action) or that a native "gentle-ai" review pass has run on this merge (this session's own `/code-review` pass is real verification but not the same second-model independent look the project's established practice uses elsewhere — still worth doing before treating the full redesign as fully validated).
