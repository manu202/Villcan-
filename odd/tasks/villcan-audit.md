...
- [ ] **`extension_in_public`** (advisor WARN, LOW) — the `citext` extension is installed in the `public` schema; should live in a dedicated schema. Not yet fixed — moving an extension already in use has some risk of breaking existing column types that reference it, worth a dedicated pass rather than folding into this batch.
...
Owner is redesigning the backoffice UI in a parallel branch (`design/visual-refresh`, synced to `main` as of `3e21dbd`, now 3 commits behind after `cbfb060`/`aea3c20`/docs — re-sync before merging). Everything below is scoped deliberately to avoid touching any `src/` component/page file, to not collide with that work. **Backend-only queue done: O-7/P-4 (`cbfb060`), O-5 (`aea3c20`).** Every item that was safely backend-only per the original UI/backend split is now closed — everything left in Money/orders (SW-O4, O-8, M-4, M-8, SW-K1-K5, SW-C4, SW-M5) genuinely needs UI (a form, a button, a page) and is on hold until the design branch's backoffice pass lands. Only the RPCs for SW-O4/O-8 (edit-fee-after-confirm, cancel-with-reason) could still be scaffolded backend-only without any UI to call them yet — worth confirming with the owner whether that's worth doing now or better done together with the UI once the design work catches up.
