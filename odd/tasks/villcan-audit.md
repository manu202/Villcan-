...
Owner is redesigning the backoffice UI in a parallel branch (`design/visual-refresh`, synced to `main` as of `3e21dbd`). Everything below is scoped deliberately to avoid touching any `src/` component/page file, to not collide with that work — see the Money/orders and Structure/hygiene sections above for the full remaining list, split by UI vs backend. **O-7/P-4 done** (see Fully closed). Still to do, backend-only: **O-5** (duplicate-order protection). RPC scaffolding for **SW-O4**/**O-8**/**M-4** (new RPCs, no UI wiring) is a reasonable next backend-only slice once O-5 closes.
