...
- [ ] **UX-1** — `/orders/new` (staff manual order creation) reuses the public storefront's own `CheckoutForm.tsx` (plain free-text nombre/teléfono/email inputs) instead of a contact search. Confirmed 2026-09-25, live QA sweep. Staff has zero visibility into whether they're matching an existing customer or creating a new one — server-side `_find_or_create_contact()` (20260901010000_orders_expansion.sql) does dedupe by **exact** phone match, so it's not creating outright garbage, but a single mistyped digit silently creates a duplicate contact with no warning, and a stale name on an existing contact can't be fixed from this flow. **Movements' own Venta flow already has the right pattern** (`movement-form/PaymentStep.tsx`: search-as-you-type against `contacts` + a "+ Crear nuevo cliente" quick-create, cliente optional) — confirmed in code (`4cedc6b`, Phase 4, predates this session entirely, not something today's merge touched or broke. If you're still seeing plain fields there live, check for a stale cache/old deploy rather than assuming the code regressed — nothing in today's work touched `PaymentStep.tsx`'s contact search.). Not fixed — this is a UX/design decision (reuse `PaymentStep`'s pattern in Orders too, vs. something else) for the owner to make before implementing, not something to unify unilaterally.
...
### 2026-09-25 (later): UX-1 found — `/orders/new` has no contact search, unlike Movements' Venta flow

The owner connected the Vercel connector to this session for an upcoming full QA pass in a fresh session, and separately reported live: "sigue saliendo un los campos para crear cliente en lugar de un buscador de cliente con la opcion de crear rapido el cliente" — for both Orders' and Movements' "new" forms. Verified before writing anything down (house rule) rather than trusting the report as-is for both halves:

- **Orders — confirmed, real gap.** `/orders/new` (`src/app/(app)/orders/new/page.tsx`) renders `CheckoutForm.tsx` — the exact same component the public storefront checkout uses (`name`/`phone`/`email` free-text inputs, no autocomplete). No contact search, no visibility into existing `contacts` rows. Server-side, `create_manual_order`'s `_find_or_create_contact()` (`20260901010000_orders_expansion.sql`) does match by exact `phone` before inserting a new contact, so it's not pure chaos — but staff can't see or pick an existing customer, can't fix a stale name from this flow, and a single mistyped phone digit creates a silent duplicate contact. Logged as **UX-1** in Current status above.
- **Movements — checked, NOT reproduced in code.** `movement-form/PaymentStep.tsx` (the Venta flow's client step) already has search-as-you-type against `contacts` plus a "+ Crear nuevo cliente" quick-create, cliente marked optional — exactly the pattern being asked for. `git log -S"Buscar cliente"` traces this to `4cedc6b` (Phase 4, the data-layer refactor), well before this session and completely untouched by anything landed today. Did not find a code-level regression here. If it's still showing plain fields live, the likeliest explanations are a stale browser cache or an old Vercel deployment being viewed rather than the current `main` — worth a hard refresh against the latest deploy before treating this half of the report as a live bug. Not written into Current status as an open item since it isn't reproduced in the code as it stands.

No fix implemented for UX-1 — flagged for the owner to decide the direction (most natural: reuse `PaymentStep`'s exact search + quick-create pattern in Orders too) before any TDD work starts on it.
