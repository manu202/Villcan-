...
### Security / auth — open
- [ ] **A-6** — invite `redirectTo` depends on a Supabase redirect allow-list that was never confirmed configured in production. **Needs owner action, not code**: no MCP/CLI tool exposes reading or writing Auth's Site URL/redirect allow-list. Check in the dashboard (Authentication → URL Configuration): the production origin (`https://villcan.vercel.app`, plus any custom domain) must be in the redirect allow-list, or an invited user can land signed-in without ever setting a password.
...
- [ ] **SW-O4** — RPC live in production since 2026-09-25 (`a221389`, `edit_confirmed_order_delivery_fee`). `design/visual-refresh` (`6ba02e1`) added a disabled, visually-distinct placeholder button in `OrderViewPanel.tsx` ("Próximamente") — **still needs the actual wiring** (call the RPC, handle the response) before this closes for real.
- [ ] **O-8** — RPC live in production since 2026-09-25 (`a221389`, `cancel_order`, requires a reason, records `cancellation_reason`/`cancelled_at`/`cancelled_by`). `design/visual-refresh` (`6ba02e1`) added a disabled placeholder ("Cancelar con motivo") next to the real, working status-select cancel path — **still needs the actual wiring** before this closes for real.
- [x] **CLOSED 2026-09-25** (`backend/audit-tickets-batch-2`, `aab7f7d`) — M-4: real `movements.expense_source` column added (nullable, scoped to `type='gasto'` by CHECK), backfilled from the existing `comment` tag, `cashBalance.ts`'s `isBankTagged` now reads the column first with comment-tag fallback for any row where it's still null. **Not fully done end-to-end**: `MovementForm.tsx` still isn't wired to WRITE the new column (out of scope for this backend-only batch, by instruction) — every row it inserts still only gets classified via the comment-tag fallback until that follow-up lands. `reports/page.tsx` also still reads comment-only (a page file, out of scope) — harmless today since `comment` keeps getting the tag from the unchanged UI, but **flagged as a real gotcha for whoever does the UI wiring**: if that pass stops writing the comment tag once it starts writing the column, `reports.ts`/`reports/page.tsx` must be updated in the same pass or Reports' balance will silently go stale.
...
- [ ] **SW-K2** — confirmed still open post visual-refresh (checked 2026-09-24 by the `design/visual-refresh` session against its own redesigned code): `src/app/(app)/page.tsx`'s Hoy/Semana/Mes toggle does move `Ingresos` breakdown and the mini-stat (both derived from `activity`, tied to `[initialized, view]`), but `Balance global`/`Balance en efectivo` come from `getRunningCashBalance(branch.id)` (~line 132), which ignores `view` entirely — always the full running balance regardless of the active tab. Pre-existing, not introduced by the redesign. `reports/page.tsx` is fine (all figures, including Balance Neto, recompute via `getDateRange(viewRef.current, ...)`).
- [ ] **SW-K3** (LOW) — confirmed still open post visual-refresh, more precise now: `page.tsx`'s "Movimientos recientes" list and the "Egresos" row DO have red/green coding (`.movement-amount--positive/--negative`, fixed `#f43f5e` on Egresos), but the two headline `Balance global`/`Balance en efectivo` numbers have no negative variant at all. `reports/page.tsx`'s "Balance Neto" has `.income`/`.expense` classes but they're just ink-tone differences (primary vs muted), not a red/green alarm signal — weak, not really addressing the finding.
...
- [x] **CLOSED 2026-09-25** (`design/visual-refresh`, `6ba02e1`) — S-7: real branch/date/text filters added over the already-fetched error list, client-side only, no new query.
...
- [ ] **Backups** — investigated 2026-09-24/25 (`b2ad713`, `docs/backups.md`): production (`villcan`, ref `vjgdtxryudoscumwsjhs`) is confirmed on the Supabase **Free plan**, which has **zero automated backups and no PITR** — there is currently nothing to restore from at all. Worse than "unverified", genuinely blocking before any real client. **Needs owner action (plan upgrade), not code.** No restore was performed or simulated (nothing exists to restore from pre-upgrade, and a real restore is a destructive production action out of scope to run unprompted). See History and `docs/backups.md` for the full finding and the restore procedure to rehearse once backups exist.
- [ ] **Production monitoring/alerting** — minimal scaffold added 2026-09-25 (`backend/audit-tickets-batch-2`, `7e7d1ab`, `docs/monitoring.md`): a `pg_cron` job flags a `client_errors` volume spike (>20/hour/branch) into a new `system_alerts` table. **Still genuinely open**: no notification channel exists (nothing emails/Slacks/pages anyone — `system_alerts` rows are only visible via direct SQL today), no `acknowledged_at` write path, no uptime check, no server-side error monitoring, no money-critical-path tagging (client_errors has no severity/category column to infer that from). See `docs/monitoring.md`'s "What this does NOT cover" section for the full honest list and concrete next steps.
...
- [ ] **`auth_leaked_password_protection`** (advisor WARN) — **needs owner action, not code**: no MCP/CLI tool exposes toggling Auth settings. Enable in the dashboard (Authentication → Policies → Password), a one-click toggle.
...
Owner is redesigning the backoffice UI in a parallel branch (`design/visual-refresh`, synced to `main` as of `3e21dbd`, now 3 commits behind after `cbfb060`/`aea3c20`/docs — re-sync before merging). Everything below is scoped deliberately to avoid touching any `src/` component/page file, to not collide with that work. **Backend-only queue done: O-7/P-4 (`cbfb060`), O-5 (`aea3c20`), SW-O4/O-8 RPC scaffolding (`a221389`).** Every item that was safely backend-only per the original UI/backend split is now closed — everything left in Money/orders (M-4, M-8, SW-K1-K5, SW-C4, SW-M5) genuinely needs UI (a form, a button, a page) and is on hold until the design branch's backoffice pass lands. SW-O4/O-8 themselves stay open (see their own lines above) until the design branch wires a UI to the new RPCs.
...
1. **SW-O4/O-8 RPC scaffolding — `a221389`.** Two new SECURITY DEFINER RPCs, backend-only, nothing calls them yet:
   ...
   - **Still open** (see Current status): no UI wires either RPC yet — that's for the design-branch backoffice pass, per the owner's original scoping note in "Package 2 (backoffice) completion, backend-only track" above.
