...
1. **Phone number is malformed on EVERY order path (storefront AND staff "Nuevo pedido") — invalid data already in production.** `src/components/storefront/CheckoutForm.tsx:39`: `phone: \`${countryCode}${phone}\`` — naive string concatenation, never strips Paraguay's local leading `0` before prepending `+595`. **`CheckoutForm` is shared** — `src/app/(app)/orders/new/page.tsx:11,132` uses the exact same component for the staff-facing manual-order screen, so this isn't storefront-only. Reproduced live twice: a storefront order (#0010, Tatapiriri, `+5950987654321`) and two staff-created orders via `/orders/new` (#0007 "QA Delivery Test", #0008 "QA Transferencia Test" — both show up in `/contacts` as `wa.me/5950981234567` / `wa.me/5950981234599`). One digit too many, an invalid number, every time a customer/staffer types the phone the natural local way (with the leading 0). This is the C-2 finding from the original audit ("normalize phone numbers on write"), now confirmed live with concrete repros instead of just inferred — and confirmed to already be sitting in real `orders.customer_phone` and `contacts.phone` rows in production right now. Ironically, a *correct* version of this exact normalization already exists and is used elsewhere: `src/lib/storefront.ts:34-41` (`digits.startsWith('0') ? '595' + digits.slice(1) : digits`) for the outbound WhatsApp status-update links — `CheckoutForm.tsx` just doesn't call it. `ContactCard.tsx:14-15`'s wa.me link builder also skips it (only strips non-digits, no normalization), so a badly-formed phone reaching `contacts.phone` propagates into a second broken link there too. **Single shared fix, three call sites to align**: reuse `src/lib/storefront.ts`'s existing normalizer in `CheckoutForm.tsx` (fixes both order-creation paths) and in `ContactCard.tsx` (fixes the display side + covers any already-bad rows already in the table).
...
6. **Storefront: two dialogs visibly stacked/overlapping.** After adding a product and opening "Ver pedido" (cart drawer) then "Confirmá tu pedido" (checkout panel), the underlying product-detail modal stays visible (dimmed) behind the newer panel instead of being fully replaced/closed — screenshotted live on `/tienda/tatapiriri`. Layering/z-index or dialog-stacking issue, not a functional blocker (checkout still completed successfully) but reads as sloppy.
...
- [ ] QA-2 Fix phone-number normalization: reuse `src/lib/storefront.ts`'s existing `startsWith('0')` normalizer in `CheckoutForm.tsx` (fixes both storefront + staff `/orders/new`) and in `ContactCard.tsx`'s wa.me link builder (finding #1).
- [ ] QA-3 Fix Reportes "Servicios" breakdown showing "Sin servicio" instead of real service names (finding #2).
- [ ] QA-4 Add missing POS option to `/orders/new`'s payment-method select (finding #3).
