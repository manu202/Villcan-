...
Data access layer (extract Supabase calls out of ~25 components into a shared layer) — **still not started**, genuinely open-ended, needs its own architecture decision (Server Components vs. client-side calls, whether to introduce Server Actions) before any code. Owner's call whether/when to take this on; everything else in the original audit plan is now done.
