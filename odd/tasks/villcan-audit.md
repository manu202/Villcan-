...
- [x] **CLOSED 2026-09-25** (`design/visual-refresh`, `fcba496`) — SW-O4: `edit_confirmed_order_delivery_fee` wired for real into `OrderViewPanel.tsx`/`src/lib/data/orders.ts` — the "Próximamente" placeholder is gone. **Note:** the branch's own task doc (`odd/tasks/backoffice-visual-refresh.md`) still had this task's checkbox unchecked when found — code-verified directly (grepped the RPC name into the diff, confirmed no placeholder text remains) rather than trusting the doc. Not yet in `main` as of this line — lives on `design/visual-refresh` (merged there `1c20087`), pending that branch's own merge to `main`.
- [x] **CLOSED 2026-09-25** (`design/visual-refresh`, `fcba496`) — O-8: `cancel_order` wired the same way, same commit, same caveat (branch's own checkbox drift, code-verified). Not yet in `main`.
- [x] **CLOSED 2026-09-25** (`backend/audit-tickets-batch-2`, `aab7f7d`, merged to `main` `913dff2`) — M-4: real `movements.expense_source` column added (nullable, scoped to `type='gasto'` by CHECK), backfilled from the existing `comment` tag, `cashBalance.ts`'s `isBankTagged` now reads the column first with comment-tag fallback for any row where it's still null. **Not fully done end-to-end**: `MovementForm.tsx` still isn't wired to WRITE the new column — every row it inserts still only gets classified via the comment-tag fallback until that follow-up lands. `reports/page.tsx` also still reads comment-only — harmless today since `comment` keeps getting the tag from the unchanged UI, but **flagged as a real gotcha for whoever does the UI wiring**: if that pass stops writing the comment tag once it starts writing the column, `reports.ts`/`reports/page.tsx` must be updated in the same pass or Reports' balance will silently go stale. Migration NOT yet applied to production (git-merged to `main` only — see the History entry on this reconciliation for exactly what "merged" does and doesn't mean here).
- [x] **CLOSED 2026-09-25** (`design/visual-refresh`, round 6, `59b0f94`) — M-8: Reportes' "Por Método" rows link to `/movements?range=&method=`, "Servicios" rows link to `/orders?range=` (date-scoped only, not per-service — an order can contain several services), both reusing Movements/Orders' own filter vocabulary rather than inventing new UI (Orders gained a real date-range filter it didn't have before, Movements gained a method sub-filter with a removable chip). "Exportar CSV" button added to Reports (client-side, no backend call). Not yet in `main` — lives on `design/visual-refresh` (merged there `1c20087`).
- [x] **CLOSED 2026-09-25** (`design/visual-refresh`, round 6, `f5902fa`) — SW-K1/K2: dashboard's Balance Global/Efectivo is a running total (since last cash closing, or all-time) and genuinely can't be scoped to the Hoy/Semana/Mes tabs the way `activity` is without answering a different, misleading question ("how much moved this week" ≠ "how much is in the drawer right now"). Resolved by making that explicit instead of faking a range-scoped number: a note under the headline balance ("Total acumulado, no varía por período") and a "Detalle del período" label above the tabs once expanded. Not yet in `main`.
- [x] **CLOSED 2026-09-25** (`design/visual-refresh`, round 6, `f5902fa`) — SW-K3: negative-amount red/green coding added to dashboard's Balance Global/Efectivo (had none) and Reports' Balance Neto (was ink-tone-only), matching `.movement-amount--positive/--negative`'s existing `#10b981`/`#f43f5e` pair. Not yet in `main`.
- [x] **CLOSED 2026-09-25** (`design/visual-refresh`, round 6, `f5902fa`) — SW-K4: added a "Facturado / Comisión" column-label row above Liquidación's breakdown (only when `commissions_enabled` and there are rows) — the commission figure had no label at all, easy to misread as a second revenue number. Not yet in `main`.
- [x] **CLOSED 2026-09-25** (`design/visual-refresh`, round 6, `f5902fa`) — SW-K5: dashboard gained a "last updated" relative-time label + manual refresh button in the header (no poll/realtime subscription exists on that page). Reports still has no equivalent — the task's own scope note said "dashboard/Reports" but Reports already re-fetches on every filter change a user makes, which is a materially different staleness story than the dashboard's fully passive load; judged not worth a matching control there without a real report of it mattering. Not yet in `main`.
...
- Whether `design/visual-refresh` should now merge to `main` — not this session's call.
