...
- [ ] **M-4** — re-scoped 2026-09-25 (confirmed by `design/visual-refresh`, `6ba02e1` commit message): `MovementForm.tsx` already has a real button-group control for cash/bank, this was never a UI gap — it's serialized into `comment` only because `movements` has no dedicated column yet. **Now backend-only**: add a real column + have the existing UI control write to it instead of tagging the comment. Safe to fold into a future backend-only batch, no UI work needed.
