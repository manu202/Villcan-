...
- [ ] **SW-O4** — RPC scaffolded backend-only 2026-09-24/25 (`a221389`, `edit_confirmed_order_delivery_fee`), **no UI wiring yet** — `OrderCard.tsx` still has no edit path after initial entry, so the user-facing bug is still open. See History.
- [ ] **O-8** — RPC scaffolded backend-only 2026-09-24/25 (`a221389`, `cancel_order`, requires a reason, records `cancellation_reason`/`cancelled_at`/`cancelled_by`), **no UI wiring yet** — there is still no cancellation button/flow calling it, and the pre-existing direct-`.update()` cancel path (no reason captured) is untouched. See History.
