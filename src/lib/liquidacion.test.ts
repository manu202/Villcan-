...
import { computeLiquidacionByBarber } from './liquidacion';

describe('computeLiquidacionByBarber (REQ-PROFIT-5)', () => {
  it('sums facturado and commission per barber across multiple movements, commissions enabled', () => {
    const result = computeLiquidacionByBarber([
  ...
  it('empty movement list -> empty result', () => {
    expect(computeLiquidacionByBarber([])).toEqual([]);
  });
